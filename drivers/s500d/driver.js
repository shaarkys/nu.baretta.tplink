...
async function validateTarget(input, credentials) {
  if (!input.ip) {
    throw new Error('An IP address is required to pair a S500D.');
  }
  ...
  if (!model.startsWith(TPLINK_MODEL) || !deviceId) {
    throw new Error('The supplied address is not an accessible S500D.');
  }
  if (input.deviceId && input.deviceId !== deviceId) {
    throw new Error(
      'The S500D at the supplied address no longer matches the device selected during discovery.',
    );
  }
    ...
    session.setHandler('get_devices', async data => {
      ...
      for (const input of inputs) {
        ...
        if (knownDeviceIds.has(target.deviceId)) {
          throw new Error('This S500D is already paired.');
        }
