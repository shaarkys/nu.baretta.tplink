...
const aes_connection_1 = __importDefault(require("./network/aes-connection"));
const klap_connection_1 = __importDefault(require("./network/klap-connection"));
...
function isAuthenticatedTransport(candidate) {
    return candidate === 'klap' || candidate === 'aes';
}
function getTransportFromEncryptType(encryptType) {
    if (typeof encryptType !== 'string')
        return undefined;
    const normalizedType = encryptType.toUpperCase();
    if (normalizedType === 'KLAP')
        return 'klap';
    if (normalizedType === 'AES')
        return 'aes';
    return undefined;
}
/**
...
class Client extends events_1.EventEmitter {
    ...
    constructor(options = {}) {
        ...
        const normalizedCredentials = (0, credentials_1.normalizeCredentialOptions)(options, 'client constructor options');
     ...
     * @internal
     */
    createConnection(transport, host, port, credentialOptions) {
        if (transport === 'udp') {
            return new udp_connection_1.default(host, port, this.log, this);
        }
        if (transport === 'klap') {
            return new klap_connection_1.default(host, port, this.log, this, {
                credentials: credentialOptions?.credentials ?? this.credentials,
                credentialsHash: credentialOptions?.credentialsHash ?? this.credentialsHash,
            });
        }
        if (transport === 'aes') {
            return new aes_connection_1.default(host, port, this.log, this, {
                credentials: credentialOptions?.credentials ?? this.credentials,
                credentialsHash: credentialOptions?.credentialsHash ?? this.credentialsHash,
            });
        }
        return new tcp_connection_1.default(host, port, this.log, this);
    }
    /**
     ...
     */
    async send(payload, host, port, sendOptions) {
        ...
        const payloadString = !(typeof payload === 'string')
            ...
            : payload;
        const connection = this.createConnection(thisSendOptions.transport, host, effectivePort, {
            credentials: this.credentials,
            credentialsHash: this.credentialsHash,
        });
        const response = await connection.send(payloadString, effectivePort, host, thisSendOptions);
        connection.close();
        return response;
    }
    /**
     ...
     */
    async sendSmart(payload, host, port, sendOptions) {
        const thisSendOptions = { ...sendOptions };
        if (thisSendOptions.transport === undefined) {
            thisSendOptions.transport = isAuthenticatedTransport(this.defaultSendOptions.transport)
                ? this.defaultSendOptions.transport
                : 'klap';
        }
        const responseString = await this.send(payload, host, port, thisSendOptions);
        const responseObj = JSON.parse(responseString);
        if (!(0, utils_1.isObjectLike)(responseObj)) {
    ...
    getBulb(deviceOptions) {
        const resolvedDeviceOptions = Client.inferDeviceTransportDefaultsFromSysInfo(deviceOptions.sysInfo, deviceOptions);
    ...
    async getDevice(deviceOptions, sendOptions) {
        this.log.debug('client.getDevice(%j)', {
            deviceOptions: (0, credentials_1.redactCredentialOptions)(deviceOptions),
            sendOptions,
        });
        let sysInfo;
    ...
    getDeviceFromSysInfo(sysInfo, deviceOptions) {
        const resolvedDeviceOptions = Client.inferDeviceTransportDefaultsFromSysInfo(sysInfo, deviceOptions);
    ...
    static inferDeviceTransportDefaultsFromSysInfo(sysInfo, deviceOptions) {
        const encryptionScheme = sysInfo.mgt_encrypt_schm;
        if (!(0, utils_1.isObjectLike)(encryptionScheme)) {
            return deviceOptions;
        }
        const inferredTransport = getTransportFromEncryptType(encryptionScheme.encrypt_type);
        const inferredPort = typeof encryptionScheme.http_port === 'number' &&
    ...
    startDiscovery(options = {}) {
        this.log.debug('client.startDiscovery(%j)', (0, credentials_1.redactCredentialOptions)(options));
