...
class TPlinkPlugDriver extends Homey.Driver {
  ...
  async onPair(session) {
    ...
    const discoverHs200Devices = (input, targetHost) => {
      stopActiveDiscovery();
    ...
    session.setHandler('discover', async data => {
      ...
      const result = await discoverHs200Devices(input);
    ...
    session.setHandler('get_devices', async data => {
      ...
      for (const input of inputs) {
        if (isIP(input.ip) !== 4) {
          throw new Error('A valid IPv4 address is required to pair an HS200 manually.');
        }

        const result = await discoverHs200Devices(input, input.ip);
        if (!pairingOpen || version !== requestVersion) return [];
        ...
        if (!target) {
          ...
          if (result.authenticationRequired) {
            throw new Error(
              'This HS200 uses authenticated firmware. Enter complete TP-Link account credentials and try again.',
            );
          }
          throw new Error(
            'No accessible HS200 was found at the supplied IP address. Check the address and try discovery again.',
          );
        }
        if (input.deviceId && input.deviceId !== target.deviceId) {
          throw new Error(
            'The HS200 at the supplied address no longer matches the device selected during discovery.',
          );
        }
        if (knownDeviceIds.has(target.deviceId)) {
          throw new Error('This HS200 is already paired.');
        }
