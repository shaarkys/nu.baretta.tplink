...
class Emeter {
    ...
    async ensureSmartSupported(sendOptions) {
    ...
    toLegacyDayStatsFromEnergyUsage(year, month, energyUsage) {
        const day = new Date().getDate();
        const dayEnergy = typeof energyUsage.today_energy === 'number' ? energyUsage.today_energy : 0;
        return {
            err_code: 0,
            day_list: [
                {
    ...
    toLegacyMonthStatsFromEnergyUsage(year, energyUsage) {
        const month = new Date().getMonth() + 1;
        const monthEnergy = typeof energyUsage.month_energy === 'number'
    ...
    toLegacyStyleAckResponse(response) {
    ...
    toSmartRealtimeFromEnergyUsage(energyUsage, currentPower) {
        const realtime = {};
        if (typeof energyUsage.current_power === 'number') {
            realtime.power_mw = energyUsage.current_power;
        }
        if (realtime.power_mw === undefined &&
    ...
    async getSmartRealtime(sendOptions) {
        await this.ensureSmartSupported(sendOptions);
        try {
            const emeterData = await this.device.sendSmartCommand('get_emeter_data', undefined, this.childId, sendOptions);
        ...
        const energyUsageResponse = await this.device.sendSmartCommand('get_energy_usage', undefined, this.childId, sendOptions);
        if (!(0, utils_1.isObjectLike)(energyUsageResponse)) {
            throw new Error(`Unexpected SMART energy usage response: ${JSON.stringify(energyUsageResponse)}`);
        }
        let currentPowerResponse;
        try {
            const response = await this.device.sendSmartCommand('get_current_power', undefined, this.childId, sendOptions);
            if ((0, utils_1.isObjectLike)(response)) {
        ...
        return this.toSmartRealtimeFromEnergyUsage(energyUsageResponse, currentPowerResponse);
    }
    /**
    ...
    async getDayStats(year, month, sendOptions) {
        if (this.isSmartPath(sendOptions)) {
            await this.ensureSmartSupported(sendOptions);
            try {
                const runtimeStats = await this.device.sendSmartCommand('get_runtime_stat', { year, month }, this.childId, sendOptions);
                if ((0, utils_1.isObjectLike)(runtimeStats)) {
            ...
            const energyUsage = await this.device.sendSmartCommand('get_energy_usage', undefined, this.childId, sendOptions);
            if (!(0, utils_1.isObjectLike)(energyUsage)) {
                throw new Error(`Unexpected SMART energy usage response: ${JSON.stringify(energyUsage)}`);
            }
            return this.toLegacyDayStatsFromEnergyUsage(year, month, energyUsage);
        }
    ...
    async getMonthStats(year, sendOptions) {
        if (this.isSmartPath(sendOptions)) {
            await this.ensureSmartSupported(sendOptions);
            try {
                const runtimeStats = await this.device.sendSmartCommand('get_runtime_stat', { year }, this.childId, sendOptions);
                if ((0, utils_1.isObjectLike)(runtimeStats)) {
                    return this.toLegacyStyleAckResponse(runtimeStats);
                }
            }
            ...
            return this.toLegacyMonthStatsFromEnergyUsage(year, energyUsage);
        }
        return this.device.sendCommand({
    ...
    async eraseStats(sendOptions) {
        if (this.isSmartPath(sendOptions)) {
            await this.ensureSmartSupported(sendOptions);
            try {
                const response = await this.device.sendSmartCommand('erase_emeter_stat', undefined, this.childId, sendOptions);
                return this.toLegacyStyleAckResponse(response);
            }
            catch {
                const response = await this.device.sendSmartCommand('erase_runtime_stat', undefined, this.childId, sendOptions);
                return this.toLegacyStyleAckResponse(response);
            }
        }
        return this.device.sendCommand({
