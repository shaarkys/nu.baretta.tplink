...
const ambient_light_1 = __importDefault(require("./ambient-light"));
...
const light_preset_1 = __importDefault(require("./light-preset"));
const light_transition_1 = __importDefault(require("./light-transition"));
const motion_1 = __importDefault(require("./motion"));
const overheat_protection_1 = __importDefault(require("./overheat-protection"));
const schedule_1 = __importDefault(require("./schedule"));
const smart_led_1 = __importDefault(require("./smart-led"));
...
class Plug extends device_1.default {
    ...
    #childComponentVersions = new Map();
    ...
    constructor(options) {
         ...
         */
        this.away = new away_1.default(this, 'anti_theft', childId);
        /**
         ...
         */
        this.ambientLight = new ambient_light_1.default(this, 'smartlife.iot.LAS', childId);
        /**
         ...
         */
        this.dimmer = new dimmer_1.default(this, 'smartlife.iot.dimmer', childId);
        this.fan = new fan_1.default(this, childId);
        this.lightPreset = new light_preset_1.default(this, childId);
        this.lightTransition = new light_transition_1.default(this, childId);
        /**
         ...
         */
        this.motion = new motion_1.default(this, 'smartlife.iot.PIR', childId);
        this.overheatProtection = new overheat_protection_1.default(this, childId);
        /**
         ...
         */
        this.emeter = new emeter_1.default(this, 'emeter', childId);
        /**
         ...
         */
        this.schedule = new schedule_1.default(this, 'schedule', childId);
        /**
         ...
         */
        this.timer = new timer_1.default(this, 'count_down', childId);
        this.smartLed = new smart_led_1.default(this, childId);
    ...
    getSysInfoTypeValue() {
        ...
        if (typeof typeValue === 'string')
            return typeValue;
        const micTypeValue = 'mic_type' in this.sysInfo ? this.sysInfo.mic_type : undefined;
        return typeof micTypeValue === 'string' ? micTypeValue : undefined;
    }
    isSmartProtocolSwitch() {
    ...
    normalizeSmartComponentList(componentList) {
        ...
        return componentList
            ...
            if ((0, utils_1.isObjectLike)(entry) && typeof entry.id === 'string') {
                return {
                    id: entry.id,
                    ver_code: typeof entry.ver_code === 'number' && Number.isInteger(entry.ver_code)
                        ? entry.ver_code
                        : 1,
                };
            }
    ...
    setChildComponentVersions(childId, componentList) {
    ...
    setChildComponentVersionsFromChildren(children) {
        children.forEach((child, childId) => {
            this.setChildComponentVersions(childId, child.components);
        });
    }
    applySmartChildDeviceList(childDeviceList, preserveStates = false) {
        if (!Array.isArray(childDeviceList)) {
            return;
        }
        childDeviceList.forEach((entry) => {
            if (!(0, utils_1.isObjectLike)(entry) || typeof entry.device_id !== 'string') {
                return;
            }
    ...
    applySmartChildComponentList(childComponentList) {
     ...
     */
    getComponentVersion(component, childId = this.#childId) {
        if (childId !== undefined) {
            const normalizedChildId = this.normalizeChildId(childId);
            const childComponentMap = this.#childComponentVersions.get(normalizedChildId);
     ...
     */
    hasComponent(component, childId = this.#childId) {
     ...
     */
    async negotiateSmartComponents(sendOptions) {
        ...
        const requests = [{ method: 'component_nego' }];
        ...
        const responses = await this.sendSmartRequests(requests, undefined, sendOptions);
        const componentNego = responses.component_nego;
        if ((0, utils_1.isObjectLike)(componentNego)) {
        ...
        if ((0, utils_1.isObjectLike)(childDeviceListResponse) &&
        ...
        if ((0, utils_1.isObjectLike)(childComponentListResponse) &&
            'child_component_list' in childComponentListResponse) {
            this.applySmartChildComponentList(childComponentListResponse.child_component_list);
        }
    ...
    get supportsFan() {
        if (this.isSmartProtocolSwitch()) {
            return (this.hasComponent('fan_control') &&
     ...
     */
    get supportsLightTransition() {
        if (this.isSmartProtocolSwitch()) {
            return (this.hasComponent('on_off_gradually') &&
     ...
     */
    get supportsOverheatProtection() {
        if (this.isSmartProtocolSwitch()) {
            return (this.hasComponent('overheat_protection') ||
    ...
    shouldUseSmartMethods(sendOptions) {
        if (!this.isSmartProtocolSwitch()) {
            return false;
        }
     ...
     * @internal
     */
    applySmartDeviceInfoPartial(partial, childId = this.#childId) {
        const normalizedChildId = childId !== undefined ? this.normalizeChildId(childId) : undefined;
        const child = normalizedChildId !== undefined
        ...
        if (this.shouldUseSmartMethods(sendOptions)) {
            if (!this.#componentsNegotiated) {
                try {
                    ...
                    this.log.debug('[%s] smart component negotiation failed: %s', this.alias, err);
                }
            }
            const response = await this.sendSmartCommand('get_device_info', undefined, this.#childId, sendOptions);
            if (!(0, utils_1.isObjectLike)(response)) {
                throw new Error(`Unexpected SMART response: ${JSON.stringify(response)}`);
            }
            this.applySmartDeviceInfoPartial(response, this.#childId);
            return this.sysInfo;
        }
    ...
    async getInfo(sendOptions) {
        if (this.shouldUseSmartMethods(sendOptions)) {
            ...
            let cloudInfo = this.cloud.info ?? { err_code: 0 };
            try {
    ...
    async setPowerState(value, sendOptions) {
        if (this.shouldUseSmartMethods(sendOptions)) {
            await this.sendSmartCommand('set_device_info', { device_on: value }, this.#childId, sendOptions);
