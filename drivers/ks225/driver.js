...
class TPlinkPlugDriver extends Homey.Driver {
    ...
    async onPair(session) {
        ...
        session.setHandler('discover', async data => {
            ...
            const finishDiscovery = () => {
                ...
                if (discoveredDevices.length > 0) {
                    this.log('Discovered ' + discoveredDevices.length + ' KS225 device(s)');
                    session.emit('discovered_devices', discoveredDevices);
                } else {
                    this.log('No KS225 devices discovered');
            ...
            const collectPlug = plug => {
                try {
                    ...
                    this.log('Error collecting discovered KS225 device: ' + err.message);
                }
            };
            ...
            discoveryClient.on('error', err => {
                this.log('KS225 discovery error: ' + err.message);
            });
            ...
            this.log(
                'Starting KS225 discovery with TP-Link account credentials: ' +
        ...
        session.setHandler('get_devices', async data => {
            ...
            this.log('Processed ' + devices.length + ' KS225 pairing device(s)');
            session.emit('continue', null);
            session.setHandler('list_devices', async () => devices);
        });

        session.setHandler('cancel', () => {
            this.log('KS225 pairing cancelled');
            stopActiveDiscovery();
        });

        session.setHandler('disconnect', () => {
            this.log('KS225 pairing finished or aborted');
            stopActiveDiscovery();
        });
    }
}
