...
exports.normalizeSmartSysInfo = exports.parseSmartDiscoveryPacket = exports.createSmartDiscoveryProbe = exports.calculateTdpCrc32 = exports.SMART_DISCOVERY_PORTS = exports.SMART_DISCOVERY_ALT_PORT = exports.SMART_DISCOVERY_PORT = void 0;
...
exports.SMART_DISCOVERY_PORT = 20002;
exports.SMART_DISCOVERY_ALT_PORT = 20004;
exports.SMART_DISCOVERY_PORTS = [
    exports.SMART_DISCOVERY_PORT,
    exports.SMART_DISCOVERY_ALT_PORT,
];
let smartDiscoveryPublicKey;
...
function getSmartDiscoveryPublicKey() {
    if (smartDiscoveryPublicKey === undefined) {
        const { publicKey } = (0, crypto_1.generateKeyPairSync)('rsa', {
            modulusLength: 2048,
 ...
 */
function calculateTdpCrc32(data) {
    let crc = 0xffffffff;
...
exports.calculateTdpCrc32 = calculateTdpCrc32;
/**
 ...
 */
function createSmartDiscoveryProbe() {
    ...
    (0, crypto_1.randomBytes)(4).copy(packet, 8);
...
exports.createSmartDiscoveryProbe = createSmartDiscoveryProbe;
/**
 ...
 */
function parseSmartDiscoveryPacket(message) {
    if (message.length < TDP_HEADER_LENGTH) {
        throw new Error('SMART discovery response is shorter than the TDP v2 header');
    }
    ...
    if (message.length !== TDP_HEADER_LENGTH + messageLength) {
        throw new Error('SMART discovery response has an invalid TDP payload length');
    }
...
exports.parseSmartDiscoveryPacket = parseSmartDiscoveryPacket;
function getSafeEncryptionScheme(input, transport, port) {
 ...
 */
function normalizeSmartSysInfo(source, options = {}) {
    ...
    const alias = decodeBase64String(source.nickname) ??
        decodeBase64String(source.device_name) ??
        getString(source.alias) ??
        model;
    const deviceOn = getBoolean(source.device_on) ?? getBoolean(source.relay_state) ?? false;
    const normalized = {
        alias,
        name: alias,
        deviceId,
        model,
        sw_ver: getString(source.sw_ver) ??
            getString(source.fw_ver) ??
            getString(source.firmware_version) ??
            '',
        hw_ver: getString(source.hw_ver) ?? getString(source.hardware_version) ?? '',
        type,
        mac,
        device_on: deviceOn,
        relay_state: deviceOn ? 1 : 0,
        mgt_encrypt_schm: getSafeEncryptionScheme(source.mgt_encrypt_schm, options.transport, options.port),
    };
    [
        'brightness',
        'fan_speed_level',
...
exports.normalizeSmartSysInfo = normalizeSmartSysInfo;
//# sourceMappingURL=smart-discovery.js.map